        ...
        if req.run_video_gen:
            ...
            if req.product_image:
                vid_result = await _proxy("POST", "video-gen", "/api/v1/video/generate", {
                    ...
                    "ugc_style": req.ugc_style or "holding_product",
...
async def script_templates():
    ...
    return {
        "durations": ["8s", "15s"],
        "ugc_styles": ["holding_product", "product_usage", "ugc_review", "talking_head"],
...
async def ugc_scripts_generate(req: dict):
    ...
    try:
        pb_result = await _proxy("POST", "prompt-builder", "/api/v1/build", {
            ...
            "ugc_style": req.get("ugc_style", "holding_product"),
        })
        if pb_result.get("ok") and pb_result.get("data"):
            data = pb_result["data"]
            ...
            ugc_style_display = {
                "holding_product": "ถือสินค้า",
                "product_usage": "ใช้สินค้า",
                "ugc_review": "รีวิว",
                "talking_head": "พูดหน้ากล้อง",
            }.get(req.get("ugc_style", "holding_product"), req.get("ugc_style", "holding_product"))
...
async def batch_create(data: dict):
    ...
    style = data.get("style", "holding_product")
