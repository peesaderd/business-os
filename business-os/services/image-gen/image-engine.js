...
async function prodiaGenerate(prompt, options = {}) {
  if (!DEFAULT_PRODIA_KEY) {
    throw new Error('PRODIA_TOKEN not configured');
  }
  ...
  // Build prompt with Thai model context
  let finalPrompt = prompt;
  if (options.thaiModel) {
    try {
      const { THAI_BASE_PROMPT } = require('./thai-model-prompt');
      finalPrompt = `${THAI_BASE_PROMPT}, ${prompt}`;
    } catch (_) { /* ignore */ }
  }
  finalPrompt += ', e-commerce product photography, professional quality, high detail';

  let aspectRatio = '1:1';
  if (options.aspectRatio === '9:16') aspectRatio = '9:16';
  else if (options.aspectRatio === '16:9') aspectRatio = '16:9';
  else if (options.aspectRatio === '4:5') aspectRatio = '4:5';
  else if (options.aspectRatio === '3:2') aspectRatio = '3:2';
  else if (options.aspectRatio === '2:3') aspectRatio = '2:3';
  ...
  for (let i = 0; i < count; i++) {
    ...
    const imgBuf = Buffer.from(view.buffer, view.byteOffset, view.byteLength);
    const hash = require('crypto').createHash('sha256').update(imgBuf).digest('hex').slice(0, 16);
    const filename = `prodia_${i}_${hash}.jpg`;
    const filePath = path.join(STORAGE_DIR, filename);
    if (!fs.existsSync(STORAGE_DIR)) {
      fs.mkdirSync(STORAGE_DIR, { recursive: true });
    }
...
async function generateImage(prompt, options = {}) {
  const provider = options.provider || (DEFAULT_PRODIA_KEY ? 'prodia' : 'fal'); // fallback prodia if key exists
  const modelTier = options.modelTier || DEFAULT_MODEL_TIER;
  const aspectRatio = options.aspectRatio;

  // Route to Prodia if specified or if PRODIA_KEY is set
